...
export class LocalConfigRollbackEngine {
  ...
  private readonly notifier: NativeRollbackNotifier;

  constructor(private readonly options: RollbackEngineOptions) {
    this.stateDirectory =
  ...
  async snapshotHostConfig(): Promise<ConfigSnapshot> {
    ...
    const snapshotPath = path.join(
      snapshotDirectory,
      `${timestampForPath(createdAt)}-${sha256.slice(0, 12)}-claude_desktop_config.json`,
    );
  ...
  async installProxyServerBlock(
    serverConfig: HostMcpServerConfig,
    serverName = this.options.proxyServerName || DEFAULT_PROXY_SERVER_NAME,
  ...
  async recordBootStartAndRollbackIfNeeded(reason = "boot_start"): Promise<RollbackResult> {
    const state = await this.readBootState();
    const next: BootLoopState = {
      ...state,
    ...
    return this.rollbackToLatestSnapshot(
      `${reason}: ${next.consecutiveBootFailures} consecutive boot failures`,
    );
  }

  async recordBootHealthy(): Promise<void> {
  ...
  async rollbackToLatestSnapshot(reason: string): Promise<RollbackResult> {
  ...
  private async purgeProxyBlockOnly(reason: string): Promise<void> {
    const configPath = await this.options.hostConfigManager.locateConfigPath();
  ...
  private async notifyRollback(reason: string): Promise<boolean> {
    try {
      await this.notifier.notify({
        title: "Hybrid Model Switcher recovered Claude Desktop config",
        body:
          "The proxy detected repeated startup failures, restored the last known-good config, and disabled the newly injected server block.",
        severity: "warning",
      });
  ...
  private async readBootState(): Promise<BootLoopState> {
    try {
  ...
  private async writeBootState(state: BootLoopState): Promise<void> {
    await fs.mkdir(this.stateDirectory, { recursive: true });
    await atomicWriteJson(this.bootStatePath, state);
  }

  private async writeLatestSnapshot(snapshot: ConfigSnapshot): Promise<void> {
    await fs.mkdir(this.stateDirectory, { recursive: true });
    await atomicWriteJson(this.latestSnapshotPath, snapshot);
  }

  private async readLatestSnapshot(): Promise<ConfigSnapshot | undefined> {
    try {
  ...
  private get bootStatePath(): string {
    return path.join(this.stateDirectory, "boot-loop-state.json");
  }

  private get latestSnapshotPath(): string {
    return path.join(this.stateDirectory, "latest-snapshot.json");
  }
}

export class TauriRollbackNotifier implements NativeRollbackNotifier {
...
export async function atomicWriteFile(filePath: string, bytes: Buffer): Promise<void> {

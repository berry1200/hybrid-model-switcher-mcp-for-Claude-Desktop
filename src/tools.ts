...
export function registerHybridTools(server: McpServer, router: HybridRouter): void {
  server.registerTool(
    "hybrid_status",
    {
      title: "Hybrid Status",
  ...
  server.registerTool(
    "hybrid_set_model",
    {
      ...
      description:
        "Persist the active MCP routing provider and model. Does not change Claude Desktop's native model selector.",
      inputSchema: {
    ...
    async ({ provider, model, autoFallback }) =>
      jsonResult(await router.setModel({ provider, model, autoFallback })),
  );
  ...
  server.registerTool(
    "hybrid_generate",
    {
      ...
      description:
        "Generate text using the active provider, an explicit Ollama model, or client sampling.",
      inputSchema: {
    ...
    async ({ prompt, system, provider, model, options }) =>
      jsonResult(
        await router.generate(
          { prompt, system, provider, model, options },
          (input) => sampleWithClient(server, input),
        ),
      ),
  );
  ...
  server.registerTool(
    ...
    async ({ messages, provider, model, options }) =>
      jsonResult(
        await router.chat(
          { messages, provider, model, options },
          (input) => sampleWithClient(server, input),
        ),
      ),
  );
  ...
  server.registerTool(
    "hybrid_recommend_model",
    {
      ...
      description:
        "Recommend a local Ollama model for reasoning, coding, balanced, chat, or fast tasks.",
      inputSchema: {

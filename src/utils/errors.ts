...
export class HybridModelError extends Error {
  constructor(
    public readonly code: HybridErrorCode | string,
    message: string,
...
export function toHostReadableError(error: unknown): HostReadableError {
  if (error instanceof McpTimeoutError) {
    return {
      code: error.code,
      message: error.message,
      hint:
        "The local route timed out. Try a smaller prompt, a faster local model, or increase HYBRID_REQUEST_TIMEOUT_MS.",
      retryable: true,
  ...
  if (error instanceof RouterConnectionError) {
    return {
      ...
      message: error.message,
      hint:
        "The local routing gateway is unavailable. Check that LiteLLM/Ollama is running and reachable.",
      retryable: true,
      details: error.details,
    };
  }
  ...
  if (error instanceof PayloadTranslationError) {
    return {
      ...
      message: error.message,
      hint:
        "The request could not be translated safely for the selected local model. Remove unsupported content blocks or tools and retry.",
      retryable: false,
      details: error.details,
    };
  }
  ...
  if (error instanceof ConfigurationError) {
    return {
      ...
      message: error.message,
      hint:
        "Check the Claude Desktop config path and the hybrid model state file permissions.",
      retryable: false,
      details: error.details,
    };
  }
...
export function safeToolErrorResult(error: unknown): CallToolResult {
...
export function parseLocalLlmFailure(error: unknown): HybridModelError {
  if (error instanceof HybridModelError) {
    return error;
  }
  ...
  if (
    ...
    return new RouterConnectionError(
      "The local model gateway is not reachable.",
      { cause: message },
    );
  }
  ...
  if (
    ...
    return new PayloadTranslationError(
      "The request exceeds the selected local model context window.",
      { cause: message },
    );
  }

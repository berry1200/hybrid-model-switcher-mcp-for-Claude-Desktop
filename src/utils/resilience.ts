...
export function createDegradationSchema(
  mode: DegradationMode,
  error: unknown,
): DegradationSchema {
  ...
  switch (mode) {
    case "local_stream_interrupted":
      return {
        ...
        mode,
        safeMessage:
          "The local model stream ended unexpectedly. The proxy kept the MCP connection open and discarded the partial buffer.",
        retryable: true,
        recoveryAction:
          "Retry the request against the fallback model or wait for the local LLM server to restart.",
    ...
    case "local_latency_degraded":
      return {
        ...
        mode,
        safeMessage:
          "Local model routing latency exceeded the configured rolling threshold. The proxy can keep the MCP connection alive while routing shifts to the configured fallback path.",
        retryable: true,
        recoveryAction:
          "Switch to the configured fallback model, reduce context size, or wait for the local route to recover.",
        fallbackProvider: "ollama",
        timestamp: new Date().toISOString(),
      };
    ...
    case "host_config_unavailable":
      return {
        ...
        mode,
        safeMessage:
          "The host configuration could not be read safely. Existing in-memory model state remains active.",
        retryable: false,
        recoveryAction:
          "Ask the user to repair claude_desktop_config.json before changing host MCP configuration.",
...
export function createFallbackUiNotification(
  degradation: DegradationSchema,
): FallbackUiNotificationSignal {
...
function normalizeErrorForMode(mode: DegradationMode, error: unknown): unknown {
  switch (mode) {
    case "cloud_route_offline":
...
function titleForMode(mode: DegradationMode): string {
  switch (mode) {
    case "local_stream_interrupted":

...
import { globalMetricsMonitor, type RouteProvider } from "../observability/metrics.js";
...
import { anthropicMessagesRequestSchema, type LiteLlmRouter } from "../routing/litellm.js";
...
export interface McpProxyPipeline {
  ...
  callTool(
    ...
    context: McpProxyPipelineContext,
...
export class CoreMcpProxyServer {
  ...
  constructor(private readonly options: CoreMcpProxyServerOptions) {
    ...
    this.server = new Server(
      {
        ...
        description:
          "MCP routing proxy for Claude Desktop, LiteLLM, Ollama, and client sampling.",
      },
      {
        capabilities: {
          tools: {
        ...
        instructions:
          "Use this proxy to discover local models, switch the active route, and explicitly route requests through Ollama or LiteLLM. It never modifies Claude Desktop internals.",
      },
    );
        ...
        async () => {
          try {
            return await withTimeout(
              this.options.pipeline.listTools(request),
              this.requestTimeoutMs,
              "tools/list handler timed out.",
            );
          } catch (error) {
      ...
      return globalMetricsMonitor.trackRequest(
        ...
        async () => {
          try {
            ...
            const operation = () =>
              this.options.pipeline.callTool(sanitizedRequest, { server: this.server });
            ...
            return await withTimeout(
              result,
              this.requestTimeoutMs,
              `tools/call handler timed out for ${sanitizedRequest.params.name}.`,
            );
          } catch (error) {
            captureTelemetryError(error, {
              code: errorCodeFromUnknown(error),
...
export class HybridMcpProxyPipeline implements McpProxyPipeline {
  constructor(private readonly deps: HybridMcpProxyPipelineDependencies) {}
  ...
  async callTool(
    ...
    switch (request.params.name) {
      ...
      case "hybrid_set_model":
        return this.asJson(
          await this.setModel(parseArguments(setModelArgsSchema, args)),
        );

      case "hybrid_generate":
        return this.asJson(
          await this.deps.router.generate(
            parseArguments(generateArgsSchema, args),
            (input) => sampleWithClient(context.server, input),
          ),
        );

      case "hybrid_chat":
        return this.asJson(
          await this.deps.router.chat(
            parseArguments(chatArgsSchema, args),
            (input) => sampleWithClient(context.server, input),
          ),
        );
      ...
      case "hybrid_recommend_model":
        return this.asJson(
          recommendModel(parseArguments(recommendArgsSchema, args).taskType),
        );
  ...
  private async routeAnthropic(args: Record<string, unknown>): Promise<CallToolResult> {
    const normalizedArgs = normalizeAnthropicRouteArgs(args);
    const parsedArgs = routeAnthropicArgsSchema.parse(normalizedArgs);
    return this.asJson(
      await this.deps.liteLlmRouter.dispatchAnthropic(parsedArgs.request, {
  ...
  private async setModel(
    input: z.infer<typeof setModelArgsSchema>,
    ...
    try {
      const legacyProvider =
        input.provider === "litellm" ? undefined : input.provider;

      const legacy =
        legacyProvider || input.model
      ...
      const active =
        input.model || input.litellmModel
      ...
      return {
        ...
        active,
        note:
          "The proxy route changed. Claude Desktop's native model selector is unchanged.",
      };
    } catch (error) {
      globalMetricsMonitor.recordModelSwitch({
        status: "failed",
  ...
  private asJson(value: unknown): CallToolResult {
    return {
      content: [
        {
          type: "text",
...
export function createHybridMcpProxyServer(
  deps: HybridMcpProxyPipelineDependencies,
  options: Omit<CoreMcpProxyServerOptions, "pipeline"> = {},
): CoreMcpProxyServer {
...
function createDiagnosticTool(error: unknown): Tool {
  return {
    name: "hybrid_proxy_diagnostic",
    title: "Hybrid Proxy Diagnostic",
    description:
      "Fallback diagnostic tool exposed when the proxy cannot enumerate its normal tool pipeline.",
    inputSchema: {
      type: "object",
      properties: {},
    },
...
function shouldRateLimitTool(name: string): boolean {
  return new Set([
    "hybrid_generate",
    "hybrid_chat",
...
function providerForTool(name: string): RouteProvider {
...
function toolResultErrorCode(result: CallToolResult): string | undefined {
...
function modelLabel(state: Readonly<ActiveLocalModelState> | undefined): string | undefined {
  if (!state) {
    return undefined;
  }
...
function requestedModelLabel(input: z.infer<typeof setModelArgsSchema>): string | undefined {
...
const CORE_PROXY_TOOLS: Tool[] = [
  {
    name: "hybrid_status",
    title: "Hybrid Status",
    description:
      "Check MCP proxy health, Ollama status, active model state, and host configuration status.",
  ...
  },
  {
    ...
    description:
      "Persist the active model route for Ollama, LiteLLM, or client sampling.",
    inputSchema: {
      type: "object",
      properties: {
        provider: {
  ...
  },
  {
    ...
    description:
      "Generate text using the active Phase 1 route: Ollama or MCP client sampling.",
    inputSchema: {
      type: "object",
      properties: {
        prompt: {
  ...
  },
  {
    ...
    description:
      "Generate a chat response using the active Phase 1 route: Ollama or MCP client sampling.",
    inputSchema: {
      type: "object",
      properties: {
        messages: {
  ...
  },
  {
    ...
    description:
      "Translate an Anthropic-style message request and dispatch it through the active LiteLLM local model route.",
    inputSchema: {
      type: "object",
      properties: {
        request: {
  ...
  },
  {
    ...
    description:
      "Recommend a local model for reasoning, coding, balanced, chat, or fast tasks.",
    inputSchema: {
      type: "object",
      properties: {
        taskType: {

import { captureTelemetryError, captureTelemetryMessage, redactSensitiveText, sanitizeTelemetryPayload, } from "../observability/telemetry.js";
import { ConfigurationError, HybridModelError, McpTimeoutError, PayloadTranslationError, RouterConnectionError, getErrorMessage, } from "../utils/errors.js";
...
export function classifyUnhandledException(error) {
    ...
    if (isFatalRuntimeError(error)) {
        return {
            ...
            retryable: false,
            message,
            operationalRunbook: "Crash loop protection must run before reconnecting the proxy; restore host config if startup fails three times.",
        };
    }
    if (error instanceof RouterConnectionError || error instanceof McpTimeoutError) {
        return {
            ...
            retryable: true,
            message,
            operationalRunbook: "Keep the MCP connection alive, emit fallback signal, and retry through the configured local fallback route.",
        };
    }
    if (error instanceof PayloadTranslationError || error instanceof ConfigurationError) {
        return {
            ...
            retryable: false,
            message,
            operationalRunbook: "Return a host-readable error, avoid process termination, and require config or payload correction.",
        };
    }
    if (error instanceof HybridModelError) {
        return {
            ...
            retryable: false,
            message,
            operationalRunbook: "Capture the structured hybrid error and keep the MCP transport open when possible.",
        };
    }
    return {
        severity: "Error",
        ...
        retryable: false,
        message,
        operationalRunbook: "Capture sanitized exception metadata, keep user payloads local, and surface a safe fallback message.",
    };
}
export function captureClassifiedException(error, context) {
...
export function createSourceMapReleasePlan(input) {
    ...
    return {
        ...
        uploadCommand: `sentry-cli sourcemaps upload ${artifactRoot} --release ${input.release} --dist ${dist}`,
        privacyControls: [
...
export function sanitizedCrashEnvelope(error) {
    const classification = classifyUnhandledException(error);
    return {
        classification,
...
//# sourceMappingURL=sentry-triage.js.map
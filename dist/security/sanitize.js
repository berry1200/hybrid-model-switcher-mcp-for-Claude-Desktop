...
const anthropicMessageSchema = z
    ...
    .strict();
const metadataSchema = z.record(z.string().min(1).max(128), z.union([
    z.string().max(4096),
    z.number(),
    z.boolean(),
    z.null(),
...
export function sanitizeToolCallRequest(request) {
    ...
    if (!parsed.success) {
        throw new PayloadTranslationError("Tool payload failed strict sanitation.", parsed.error.flatten());
    }
    return {
        ...request,
        params: {
            ...request.params,
            arguments: deepFreeze(deepClone(parsed.data)),
        },
    };
}
export function assertJsonSafe(value, path, depth = 0) {
    if (depth > MAX_DEPTH) {
    ...
    if (typeof value === "string") {
        ...
        if (/[\u0000\u001b]/u.test(value)) {
            throw new PayloadTranslationError("Tool payload contains forbidden control characters.", { path });
        }
        return;
    }
    ...
    if (typeof value === "object") {
        ...
        if (prototype !== Object.prototype && prototype !== null) {
            throw new PayloadTranslationError("Tool payload contains a non-plain object.", { path });
        }
        for (const [key, item] of Object.entries(value)) {
            if (FORBIDDEN_KEYS.has(key.toLowerCase())) {
                throw new PayloadTranslationError("Tool payload contains a forbidden key associated with shell execution or prototype pollution.", { path: `${path}.${key}`, key });
            }

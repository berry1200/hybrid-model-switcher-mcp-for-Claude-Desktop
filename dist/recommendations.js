...
export function recommendModel(taskType) {
    const normalized = taskType.trim().toLowerCase();
    return (MODEL_RECOMMENDATIONS.find((item) => item.taskType === normalized ||
        item.displayName.toLowerCase() === normalized ||
        item.modelName.toLowerCase().startsWith(normalized)) ?? MODEL_RECOMMENDATIONS[2]);
}

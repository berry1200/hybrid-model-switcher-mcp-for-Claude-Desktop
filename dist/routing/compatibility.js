...
const runtimeConfigSchema = z
    ...
    .strict();
export function migrateRuntimeConfig(raw, defaults) {
    ...
    if (record.schemaVersion === CURRENT_COMPATIBILITY_SCHEMA_VERSION) {
        ...
        if (!parsed.success) {
            throw new ConfigurationError("Current runtime config failed compatibility validation.", parsed.error.flatten());
        }
        return parsed.data;
    }
    return runtimeConfigFromLegacy(record, defaults);
}
export function migrateActiveModelState(raw) {
    if (!raw || typeof raw !== "object") {
        return createActiveModelState({});
    }
    return createActiveModelState(raw);
}
export function normalizeAnthropicRouteArgs(raw) {
    if (!raw || typeof raw !== "object") {
        throw new PayloadTranslationError("Tool arguments must be an object.");
    }
    const record = raw;
    const requestCandidate = record.request ?? record.payload ?? record.anthropic ?? record.messagesRequest;
    if (!requestCandidate || typeof requestCandidate !== "object") {
        throw new PayloadTranslationError("Anthropic route arguments did not include a request payload.");
    }
    const request = normalizeAnthropicMessagesRequest(requestCandidate);
    return {
        request,
        modelOverride: normalizeOptionalString(record.modelOverride ?? record.model ?? record.localModel),
        fallbackModelOverride: normalizeOptionalString(record.fallbackModelOverride ?? record.fallbackModel ?? record.fallback),
    };
}
export function normalizeAnthropicMessagesRequest(raw) {
...
export function normalizeLiteLlmMetadata(metadata, routing) {
...
function runtimeConfigFromLegacy(record, defaults) {
    ...
    const litellmModel = normalizeOptionalString(routingRecord.litellmModel ?? routingRecord.litellm_model ?? routingRecord.liteLlmModel);
    return {
        schemaVersion: CURRENT_COMPATIBILITY_SCHEMA_VERSION,
        endpoints: {
            ollamaBaseUrl: normalizeOptionalString(endpointsRecord.ollamaBaseUrl ?? endpointsRecord.ollama_url) ??
                defaults.ollamaBaseUrl,
            liteLlmBaseUrl: normalizeOptionalString(endpointsRecord.liteLlmBaseUrl ??
                endpointsRecord.litellmBaseUrl ??
                endpointsRecord.litellm_url) ?? defaults.liteLlmBaseUrl,
        },
        routing: {
            provider,
            model,
            litellmModel,
            fallbackModel: normalizeOptionalString(routingRecord.fallbackModel ?? routingRecord.fallback_model),
            autoFallback: typeof routingRecord.autoFallback === "boolean"
...
function createActiveModelState(record) {
    return {
        provider: normalizeProvider(record.provider),
        model: normalizeOptionalString(record.model),
        litellmModel: normalizeOptionalString(record.litellmModel ?? record.litellm_model ?? record.liteLlmModel),
        fallbackModel: normalizeOptionalString(record.fallbackModel ?? record.fallback_model),
        autoFallback: typeof record.autoFallback === "boolean"
            ? record.autoFallback
        ...
        revision: typeof record.revision === "number" && Number.isInteger(record.revision)
            ...
            : 0,
        updatedAt: normalizeOptionalString(record.updatedAt ?? record.updated_at) ??
...
function normalizeLegacySystem(value) {

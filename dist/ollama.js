...
export class OllamaConnector {
    ...
    async fetchWithTimeout(endpoint, init) {
        ...
        try {
            ...
            if (!response.ok) {
                ...
                throw new HybridModelError("ollama_request_failed", `Ollama request failed with ${response.status} ${response.statusText}`, body);
            }
            return response;
        }
        catch (error) {
            ...
            throw new HybridModelError("ollama_unavailable", `Could not reach Ollama at ${this.config.ollamaBaseUrl}`, error);
        }
        finally {
